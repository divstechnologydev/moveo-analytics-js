(function (window) {
  ...
  class MoveoOneWeb {
    ...
    // Helper method to get stable text content from an element
    ...
    getStableAttributes(element) {
      ...
      // ARIA attributes that are set in HTML
      ...
      // Data attributes that are set in HTML
      ...
      if (dataAttributes.length > 0) {
        ...
        attributes[dataAttr.name] = dataAttr.value;
      }
    ...
    getStableContent(element) {
      ...
      if ((element.tagName === 'VIDEO' || element.tagName === 'AUDIO') && element.src) {
        content.src = element.src;
      }
    ...
    // Alternative: Persistent ID storage using localStorage (PRIVACY CONCERN - use with caution)
    // This method stores element IDs in localStorage to persist across sessions
    // WARNING: This may store sensitive information and should be used carefully
    /*
    generatePersistentElementId(element) {
      const storageKey = 'moveo-element-ids';
      ...
      try {
        ...
        // Check if we already have an ID for this element signature
        if (existingIds[elementSignature]) {
          return existingIds[elementSignature];
        }
        
        // Generate new ID
        const newId = this.generateStableElementId(element);
        
        // Store the mapping
        existingIds[elementSignature] = newId;
        localStorage.setItem(storageKey, JSON.stringify(existingIds));
        
        return newId;
      } catch (error) {
        // Fallback to non-persistent method if localStorage fails
        console.warn('MoveoOne: localStorage failed, falling back to non-persistent IDs');
        return this.generateStableElementId(element);
      }
    }
    ...
    getElementSignature(element) {
      ...
      return this.hashString(JSON.stringify(signature));
    }
    */
  }
