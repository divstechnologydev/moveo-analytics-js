(function (window) {
  ...
  const API_URL = "http://127.0.0.1:8000/api/analytic/event/tag";
  const LIB_VERSION = "1.0.2"; // Constant library version - cannot be changed by client
  ...
  class MoveoOneWeb {
    constructor(token) {
      ...
      // Element ID cache for stable identifiers
    ...
    async start() {
      ...
      if (this.isExistingSession) {
        ...
        this.track("page_view", {
          ...
          id: this.generateGlobalEventId("page_view", { path: window.location.pathname }),
          type: "page",
          action: "view",
        ...
        this.track("viewport_size", {
          semanticGroup: "global",
          id: this.generateGlobalEventId("viewport_size", { path: window.location.pathname }),
          type: "viewport",
          action: "size",
    ...
    getSemanticGroup(element) {
      ...
      // Define semantic elements in priority order (excluding section which we check first)
      const otherSemanticElements = ['article', 'aside', 'nav', 'header', 'footer', 'main'];
      ...
      // If no section found, look for other semantic elements
      ...
      // If no semantic parent found, look for first div with id
      if (!semanticParent) {
        currentElement = element;
        while (currentElement && currentElement !== document.body) {
          const tagName = currentElement.tagName.toLowerCase();
    ...
    getSemanticGroupName(element, includeElementName) {
      ...
      // Priority 2 - For non-section elements, use document title instead of CSS classes
      if (includeElementName) {
        // This is a non-section semantic element, use document title
        const documentTitle = document.title || "page";
        const cleanTitle = this.cleanSemanticGroupName(documentTitle);
        return `${tagName}_${cleanTitle}`;
      } else {
        // This is a section element, use CSS classes as before
        if (element.className && element.className.trim()) {
          const classNames = element.className.split(' ').filter(cls => cls.trim());
          ...
          const meaningfulClasses = classNames.filter(cls => {
            ...
            return !lowerCls.match(/^(bg-|text-|p-|m-|w-|h-|flex|grid|block|inline|hidden|visible|opacity|border|rounded|shadow|transition|transform|hover|focus|active|disabled|container|row|col|btn|card|modal|nav|header|footer|sidebar|main|content|wrapper|section|article|aside)$/);
          });
          ...
          const nonGenericClasses = classNames.filter(cls => {
            const lowerCls = cls.toLowerCase();
            return !lowerCls.match(/^(container|wrapper|content|section|main|header|footer|nav|sidebar|row|col|btn|card|modal)$/);
          });
    ...
    async trackPageNavigation() {
      ...
      const event = {
        ...
        prop: {
          sg: "global",
          eID: this.generateGlobalEventId("page_view", { path: window.location.pathname }),
          eA: "view",
          eT: "page",
          eV: window.location.pathname || "",
        },
        sId: this.sessionId,
      };
      ...
      document.addEventListener("visibilitychange", () => {
        if (document.visibilityState === "hidden") {
          ...
          this.track("visibility_change", {
            semanticGroup: "global",
            id: this.generateGlobalEventId("visibility_change", { state: "hidden" }),
            type: "visibility",
            action: "hidden",
            value: "page_hidden",
          });
          ...
          this.track("visibility_change", {
            semanticGroup: "global",
            id: this.generateGlobalEventId("visibility_change", { state: "visible" }),
            type: "visibility",
            action: "visible",
            value: "page_visible",
          });
      ...
      document.addEventListener("copy", (e) => {
        ...
        if (text && text.trim().length > 0) {
          ...
          this.track("copy", {
            ...
            id: anchorElement ? this.generateStableElementId(anchorElement) : "copy_text",
            type: "clipboard",
            action: "copy",
      ...
      window.addEventListener("scroll", () => {
        ...
        scrollTimeout = setTimeout(() => {
          ...
          if (Math.abs(scrollPosition - lastScrollPosition) > 100) {
            this.track("scroll", {
              semanticGroup: "global",
              id: this.generateGlobalEventId("scroll", { scrollPercentage: scrollPercentage }),
              type: "scroll",
              action: "scroll",
    ...
    generateStableElementId(element) {
      // Check if we already have a cached ID for this element
      if (this.elementIdCache.has(element)) {
        return this.elementIdCache.get(element);
      }

      // For elements with stable IDs, use them directly
      if (element.id && element.id.trim()) {
        const stableId = this.cleanElementId(element.id);
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For form elements with names, use the name
      if (element.name && element.name.trim()) {
        const stableId = this.cleanElementId(element.name);
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For links, use a hash of the href
      if (element.href && element.tagName === 'A') {
        const hrefHash = this.hashString(element.href);
        const stableId = `link_${hrefHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For images, use a hash of the src
      if (element.src && element.tagName === 'IMG') {
        const srcHash = this.hashString(element.src);
        const stableId = `img_${srcHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For media elements, use a hash of the src
      if (element.src && (element.tagName === 'VIDEO' || element.tagName === 'AUDIO')) {
        const srcHash = this.hashString(element.src);
        const stableId = `${element.tagName.toLowerCase()}_${srcHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with meaningful text content, create a hash of the text
      const textContent = this.getElementText(element);
      if (textContent && textContent.trim().length > 0) {
        const textHash = this.hashString(textContent);
        const stableId = `${element.tagName.toLowerCase()}_${textHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with aria-label, use a hash of the aria-label
      if (element.getAttribute('aria-label')) {
        const ariaLabel = element.getAttribute('aria-label');
        const labelHash = this.hashString(ariaLabel);
        const stableId = `${element.tagName.toLowerCase()}_aria_${labelHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with title attribute, use a hash of the title
      if (element.title) {
        const titleHash = this.hashString(element.title);
        const stableId = `${element.tagName.toLowerCase()}_title_${titleHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with alt attribute (images), use a hash of the alt text
      if (element.alt) {
        const altHash = this.hashString(element.alt);
        const stableId = `${element.tagName.toLowerCase()}_alt_${altHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with value attribute (inputs), use a hash of the value
      if (element.value && element.value.trim()) {
        const valueHash = this.hashString(element.value);
        const stableId = `${element.tagName.toLowerCase()}_value_${valueHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with placeholder, use a hash of the placeholder
      if (element.placeholder) {
        const placeholderHash = this.hashString(element.placeholder);
        const stableId = `${element.tagName.toLowerCase()}_placeholder_${placeholderHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with role attribute, use the role
      if (element.getAttribute('role')) {
        const role = element.getAttribute('role');
        const stableId = `${element.tagName.toLowerCase()}_role_${role}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with data attributes, try to find a meaningful one
      const dataAttributes = Array.from(element.attributes)
        .filter(attr => attr.name.startsWith('data-'))
        .filter(attr => attr.value && attr.value.trim().length > 0);

      if (dataAttributes.length > 0) {
        // Use the first data attribute with a value
        const dataAttr = dataAttributes[0];
        const dataValueHash = this.hashString(dataAttr.value);
        const stableId = `${element.tagName.toLowerCase()}_${dataAttr.name}_${dataValueHash}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // For elements with specific types (inputs), use the type
      if (element.type) {
        const stableId = `${element.tagName.toLowerCase()}_${element.type}`;
        this.elementIdCache.set(element, stableId);
        return stableId;
      }

      // Last resort: use element position in DOM tree
      const domPath = this.getElementPath(element);
      const pathHash = this.hashString(domPath);
      const stableId = `${element.tagName.toLowerCase()}_path_${pathHash}`;
      this.elementIdCache.set(element, stableId);
      return stableId;
    }
    ...
    generateGlobalEventId(eventType, additionalData = {}) {
      ...
      if (eventType === 'viewport_resize' && additionalData.width && additionalData.height) {
