(function (window) {
  ...
  class MoveoOneWeb {
    ...
    // Immediate track method for critical events (like downloads)
    ...
    setupDownloadTracking() {
      // Function to track existing download links
      const trackDownloadLinks = () => {
        const downloadLinks = document.querySelectorAll("a[href]");

        downloadLinks.forEach((link) => {
          // Skip if already tracked
          if (link.dataset.moveoDownloadTracked) return;
          ...
          if (
            ...
            link.dataset.moveoDownloadTracked = "true";
            ...
            link.addEventListener("click", (event) => {
              ...
              // Track download immediately using sendBeacon for reliability
              this.trackImmediate("download", {
                semanticGroup: "resource",
                id: filename,
                type: "download",
                action: "click",
                value: filename,
      ...
      // Track download links on initial load
      if (document.readyState === "loading") {
        document.addEventListener("DOMContentLoaded", trackDownloadLinks);
      } else {
        trackDownloadLinks();
      }

      // Watch for dynamically added download links
      const setupDownloadObserver = () => {
      ...
      if (document.body) {
        ...
        document.addEventListener("DOMContentLoaded", setupDownloadObserver);
      }
    }
    ...
    setupClickTracking() {
      document.addEventListener("click", (event) => {
        ...
        // Skip download links as they're handled separately
        if (
          target.tagName === "A" &&
          target.href &&
          target.href.match(
        ...
        // Skip download links found in parent search
        if (
          target.tagName === "A" &&
          target.href &&
          target.href.match(
