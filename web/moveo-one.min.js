  ...
  class MoveoOneWeb {
    ...
    setupClickTracking() {
      document.addEventListener("click", (event) => {
        ...
        // Use immediate tracking for navigation links to ensure proper order
        if (target.tagName === "A" && target.href) {
          // Check if this is a navigation link (same domain but different page)
          try {
            const linkUrl = new URL(target.href, window.location.href);
            const currentUrl = new URL(window.location.href);
            
            // If same domain but different path, it's a navigation link
            ...
            // If URL parsing fails, fall back to buffered tracking
          }
        }
    ...
    getStableAttributes(element) {
      ...
      if (element.className && element.className.trim()) {
        ...
        if (meaningfulClasses.length > 0) {
          // Take the first meaningful class
          attributes.meaningfulClass = this.cleanSemanticGroupName(meaningfulClasses[0]);
        }
      }
