(function (window) {
  ...
  class MoveoOneWeb {
    ...
    initialize() {
      ...
      this.setupOutboundAndDownloadTracking();
    }
    ...
    setupOutboundAndDownloadTracking() {
      // Get current domain for outbound link detection
      const currentDomain = window.location.hostname;

      // File extensions to track as downloads
      const downloadExtensions =
        /\.(pdf|zip|docx?|xlsx?|pptx?|txt|csv|json|xml|rar|7z|tar|gz|exe|dmg|pkg|deb|rpm|mp3|mp4|avi|mov|wmv|flv|webm|ogg|wav|flac|jpg|jpeg|png|gif|svg|webp|bmp|tiff?|ico|psd|ai|eps|sketch)$/i;

      // Track all link clicks for outbound and download detection
      document.addEventListener("click", (event) => {
        ...
        try {
          ...
          const isDownload = downloadExtensions.test(fileName);
          const isOutbound =
            linkDomain !== currentDomain && linkUrl.protocol.startsWith("http");
          ...
          if (isDownload) {
            ...
            this.track("download", {
              ...
              type: "download",
              action: "click",
              value: target.textContent?.trim() || fileName,
            });
          } else if (isOutbound) {
            // Track outbound link
            this.track("outbound_link", {
              ...
              type: "outbound_link",
              action: "click",
              value: target.textContent?.trim() || target.href,
            });
          }
        } catch (error) {
          // Handle invalid URLs gracefully
          console.warn(
            "MoveoOne: Invalid URL in link tracking:",
            target.href,
            error
          );
        }
