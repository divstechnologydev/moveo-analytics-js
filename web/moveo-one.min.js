(function (window) {
  ...
  class MoveoOneWeb {
    ...
    getSemanticGroup(element) {
      ...
      const otherSemanticElements = ['article', 'aside', 'nav', 'header', 'footer'];
    ...
    getSemanticGroupName(element, includeElementName) {
      ...
      if (element.className && element.className.trim()) {
        ...
        const meaningfulClasses = classNames.filter(cls => {
          ...
          return !lowerCls.match(/^(bg-|text-|p-|m-|w-|h-|flex|grid|block|inline|hidden|visible|opacity|border|rounded|shadow|transition|transform|hover|focus|active|disabled|container|row|col|btn|card|modal|nav|header|footer|sidebar|main|content|wrapper|section|article|aside|w-|w-embed|w-script|w-dyn-|w-tab-|w-form-)$/);
        });
    ...
    getStableAttributes(element) {
      ...
      // Data attributes that are set in HTML (but exclude Webflow's w-id)
      const dataAttributes = Array.from(element.attributes)
        .filter(attr => attr.name.startsWith('data-'))
        .filter(attr => attr.value && attr.value.trim().length > 0)
        .filter(attr => !attr.name.includes('w-id')); // Exclude Webflow's dynamic IDs
